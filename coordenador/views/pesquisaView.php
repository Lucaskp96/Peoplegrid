   ...
   <?=begin_ToolBar(array('imprimir', 'abrir', 'ajuda', 'pesquisar', 'novo'));?>
    ...
    <?=begin_TabPanel();?>
        <?=begin_Tab(lang('pesquisaPesquisa'));?>
            <?=begin_form('coordenador/pesquisa/salvar', 'formPesquisa');?>
...
<script>
    ...
    function filtro(){
        location.href = BASE_URL+'coordenador/pesquisa';
    }
    ...
    function excluirPesquisa(confirmaExclusao){
        if (confirmaExclusao) {
            $.post(BASE_URL+'coordenador/pesquisa/excluir', {pesquisas: $("#txtId").val()},
            function(data){
                if (data.success) {
    ...
    function gridPerguntas_loadComplete(){
        ...
        $.post(BASE_URL+'coordenador/pesquisa/listaQuestionarioPerguntas', {questionarioId: questionarioId},
        function(data){
