<?= headerView() ?>
    ...
    <?=begin_TabPanel();?>
        <?=begin_Tab(lang('perguntasTab'));?>
            <?=begin_form('coordenador/perguntas/salvar', 'formPergunta');?>
...
<script type="text/javascript">
    ...
    function filtro(){
        location.href = BASE_URL+'coordenador/perguntas';
    }
    ...
    function excluirPergunta(confirmaExclusao){
        if (confirmaExclusao) {
            $.post(BASE_URL+'coordenador/perguntas/excluir', {id: $("#txtId").val()},
            function(data){
                if (data.success) {
