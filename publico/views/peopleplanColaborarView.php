                ...
                <div id="perguntas_descricao" class="row">
                    <div class=" col-xs-1" >
                        <div id="btnAnterior" class="btn" onclick="anteriorPergunta()"> <i class="fa fa-chevron-circle-left fa-5x"> </i></div>
                         <div id="btnVoltaPerguntas" class="btn" onclick="voltaPerguntas()"> <i class="fa fa-chevron-circle-left fa-5x"> </i></div>
                    </div>
                    ...
                    <div class=" col-xs-1" style="margin-left:-30px">
                        ...
                        <div id="btnProximoQuestionario" class="btn" onclick="proximoQuestionario()"><i class="fa fa-chevron-circle-right fa-5x"></i></div>
...
<div id="row_ferramentas_mapa" class="row-fluid">
    ...
    <div id="ferramentas" class="col-xs-2">
        ...
        <hr>
            <div>
                <center>Opções de grid</center>
            </div>
            <div id="opcoes">
                <div class="radio" id="gridopcoes">
                    <label><input class="gridopcoes" onclick="opcoesGrid()" type="radio" name="optradio" id="o0" value=0 checked>Grid preta</label>
                </div>
                <div class="radio" id="gridopcoes">
                    <label><input class="gridopcoes" onclick="opcoesGrid()" type="radio" name="optradio" id="o1" value=1>Grid branca</label>
                </div>
                <div class="radio" id="gridopcoes">
                    <label><input class="gridopcoes" onclick="opcoesGrid()" type="radio" name="optradio" id="o2" value=2>Grid transparente</label>
                </div>
            </div>
    ...
    <div id="ponderadorPeso" class="col-xs-2" >
            <span>
                <p id="lblFerramentas" class="text-center">Poderador Pesos</p>
            </span>
            <div id="linha" class="text-center"></div>
                <?php $name = "a";
                            foreach($questionarioPerguntas as $p){?>
                            <div class="control-group" >
                                <label class="control-label h4" style="text-align:center;margin: 10px 7.5% 10px 7.5%;"><?=$p->descricao?><input name="ponderador" type="range" min="1" max="10" value="<?=$p->peso?>" onchange="<?=$name?>.value=value">
                                <output style="text-align: center;font-size: 20px;" id="<?=$name?>"><?=$p->peso?></output></input></label>
                            </div>
                            <?php
                            $name .= "a";
                        }
    ...
    <div id="mapa" class="col-xs-10" style="visibility: hidden;"> 
...
<script>
    ...
    $(document).ready(function() {
        ...
        var infoTutorial = document.getElementById('info_tutorial');
        infoTutorial.style.fontFamily = 'Helvetica';      //fonte do tutorial
        ...
        map.style.visibility="visible";  //setando a div mapa visivel depois de carregar
    ...
    function init() {
        ...
        $("#perguntasGrid").show();
        $('#btnAnterior').hide();
        $('#btnProximoQuestionario').hide();
        $('#btnVoltaPerguntas').hide();
        $('#success').hide();
        $('#fail').hide();
        $('#btnEnviar').hide();
    ...
    //funcao para mudar cor da grid
    var opt;
    function opcoesGrid(){
        var gridEscolhida = document.getElementsByName('optradio');
        var escolhaValor;
        for(var i=0; i < gridEscolhida.length; i++){
            if(gridEscolhida[i].checked){
                escolhaValor = gridEscolhida[i].value;
            }
        }
        ...
        for (var i = 0; i < rectArr.length; i++) {
            switch(escolhaValor){
                ...
                case '2':
                    ...
                    break;
                default:
                    messageBox("Selecione um tipo de grid.");
            }   
        }
    }
    ...
    function manipulaFormulario() {
        ...
        if (questaoCorrente <= perguntas.length) {
            ...
            $("#ferramentas").hide();
            $("#ponderador").show();
            $("#contagem").html("<h3>Questionario Pesos</h3>");
            $("#pergunta").html("<small> Uma forma de você informar quais critérios julga mais importantes para este questionário.<br>Suas respostas serão sempre anônimas.</small>");
            $("#ponderadorPeso").show();
            $("#info_tutorial").hide();
            $("#interrogacao").hide();            
            $("#perguntasObjetivas").hide();
            $('#btnProximo').hide();
            $('#btnProximoQuestionario').show();
    ...
    function enviar() {
        ...
        $.post(BASE_URL + 'publico/peopleplanhome/salvar',
                {
        ...
        function(data) {
            ...
            if (result.success.message === 'success') {
                ...
                $('#aviso_botao_resultado').click(function() {
                    ...
                    location.href = BASE_URL+"publico/peopleplanhome/apuracao/"+ $('#txtQuestionarioLink').val();
