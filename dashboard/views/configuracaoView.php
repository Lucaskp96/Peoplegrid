	...
	<?=begin_TabPanel();?>
		<?=begin_Tab(lang('configuracaoTab0'));?>
			<?=begin_form('dashboard/configuracao/salvarGeral', 'formGeral');?>
                                <?=form_hidden('txtPessoaId', getUsuarioSession()->pessoa_id, 103, '');?>
				<?=form_label('lblNome', 'Nome', 100);?>
				...
				<?=form_textField('txtLogin', @$pesquisador->login, 103, '');?>
				<?=new_line();?>
                                <?=new_line();?>
                                <?=form_label('lblEmail', 'Email', 100);?>
				<?=form_textField('txtEmail', @$pesquisador->email, 250);?>
