 ...
 * @author fdloopes
 */
class PeopleplanQuestionarioModel extends Model {
    ...
    /**
     * Busca todos os questionarios a disposição do resondente
     * 
    ...
    function getQuestionariosHome() {
        ...
        $this->db->from('peopleplan.questionario_perguntas_multicriterio as qpg');
...
/*
  ...
  */      
        ...
        $this->db->insert('peopleplan.questionarios');
        $this->db->trans_complete();

        $id = $this->db->insert_id('peopleplan.questionarios', 'id');
        ...
        if(($perguntas['txtFlag'] == 1) && ($ret)){
            ...
            if ($perguntas['txtPerguntas'] != '') {
                ...
                for ($i = 0; $i < count($perguntasId); $i++) {
                    ...
                    $this->db->where('pergunta_id', $perguntasId[$i]);
                    $this->db->update('peopleplan.questionario_perguntas_multicriterio');
                    logLastSQL();
                }
            }
            //logVar($perguntas);
            ...
            if ($perguntas['txtPerguntas'] != '') {
                ...
                for ($i = 0; $i < count($perguntasId); $i++) {
                    ...
                    $this->db->set('pergunta_id', $perguntasId[$i]);
                    $this->db->set('ordem', $i + 1);
                    $this->db->set('dt_cadastro', 'now()');
                    $this->db->insert('peopleplan.questionario_perguntas_multicriterio');
                }
            }
        }
        if ($this->db->trans_status() === FALSE) {
    ...
    function getRespostasByQuestionarioPerguntaGridId($idPerguntaQuestionario) {
        ...
        $this->db->where('gr.questionario_pergunta_id', $idPerguntaQuestionario);
        $data = $this->db->get()->result();
        return $data;
    ...
    function validaPerguntaQuestionario($perguntas) {
        $this->db->trans_begin();
            ...
            $this->db->from('peopleplan.questionario_perguntas_multicriterio as qp');
            $this->db->where('qp.questionario_id', @$perguntas['txtIdQuestionarioPerguntas']);
            $total = $this->db->get('peopleplan.questionario_perguntas_multicriterio')->row()->total_perguntas;
        $this->db->trans_complete();
    ...
    function validaRespostasPerguntaQuestionario($idPerguntaQuestionario) {
        $this->db->trans_begin();
            ...
            $this->db->from('peopleplan.respostas_multicriterio as gr');
            $this->db->where('gr.questionario_pergunta_id', $idPerguntaQuestionario);
            $total = $this->db->get('peopleplan.respostas_multicriterio')->row()->total_respostas;
        $this->db->trans_complete();
