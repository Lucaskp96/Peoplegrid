...
-->
            <?=begin_Tab(lang('questionario'));?>
                    <?=begin_form('pesquisador/peopleplanQuestionario/salvarQuestionario', 'formQuestionario');?>
...
-->
            <?=begin_Tab("Local da Pesquisa");?>        
                <?=begin_form('pesquisador/peopleplanQuestionario/salvarReferenciaMapa', 'formMapa');?>
...
-->
            <?=begin_Tab("Criar Grid");?>
                <?=begin_form('pesquisador/peopleplanQuestionario/salvarGrid', 'formGrid');?>
...
=====================================================================================       
            ...
            <?=begin_Tab(lang('questionarioPerguntasPesos'));?>
                <style>
                    #lista3{
                ...
                <h2 style="text-align: center"> <?= lang('questionarioPesosAviso')?></h2>
                    
                <?=new_line();?>
                <?= hr(); ?>
                <?=new_line()?>
                <?=begin_form('pesquisador/peopleplanQuestionario/salvarPesosQuestionario', 'formPesos');?>
                    <?=form_hidden('txtPessoaId', @$questionario->pessoa_id);?>
                    <?=form_hidden('txtIdQuestionarioPerguntas', @$questionario->id);?>
                    <?=form_hidden('txtPerguntas','');?>
                    <?=form_hidden('txtFlag','0');?>
                    ...
                    '<ul id="lista3" style="float:center; width: 100%; text-align:center;min-height:120px;">';

                        $name = "a";
                        if(isset($perguntasQuestionario)){
                            foreach($perguntasQuestionario as $pq){?>
                                <label style="margin: 10px 7.5% 10px 7.5%;"><h2><?=$pq->descricao?></h2><input name="ponderador" type="range" min="1" max="10" value="<?=$pq->peso?>" onchange="<?=$name?>.value=value">
                                <h3 style="font-size: 15px;"><output id="<?=$name?>"><?=$pq->peso?></output></h3></input></label>
...
-->                        
            <?=begin_Tab(lang('abaQuestionarioAtivo'));?>
                <?=begin_form('pesquisador/peopleplanQuestionario/salvarQuestionarioAtivo', 'formQuestionarioAtivo');?>
 ...
 */ 
    ...
    $(document).ready(function(){
        ...
        /*$("#ui-id-6").click(function(){
                $('#txtIdQuestionarioPerguntas').val($('#txtId').val());
                aviso.setMessageWarning('<?= lang('questionarioEtapa5') ?>');
                estadoInicialPesos = $( "#lista3" ).sortable( "toArray" );
    ...
    function novaPergunta(){
        ...
        location.href = BASE_URL+'pesquisador/peopleplanPergunta/novo';
    }
 ...
 */
    ...
    function novo(){
        location.href = BASE_URL+'pesquisador/peopleplanQuestionario/novo';
    }
    ...
    function excluirQuestionario(confirmaExclusao){
        if (confirmaExclusao) {
            $.post(BASE_URL+'pesquisador/peopleplanQuestionario/excluir', {questionarios: $("#txtId").val()},
            function(data){
                if (data.success) {
 ...
 */    
    ...
    function filtro(){
        location.href = BASE_URL+'pesquisador/peopleplanQuestionario';
    }
    ...
    function gridPerguntas_loadComplete(){
        ...
        $.post(BASE_URL+'pesquisador/peopleplanQuestionario/listaQuestionarioPerguntas', {questionarioId: questionarioId},
        function(data){
