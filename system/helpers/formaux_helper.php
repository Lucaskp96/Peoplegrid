 ...
 */
function form_checkbox($name = '', $id = '', $value = '', $checked = FALSE, $extra = '', $disabled = false, $label = '', $label_extras = array(), $onClick = '') {
    $defaults = array('type' => 'checkbox', 'name' => $name, 'value' => $value, 'onClick' => $onClick);
...
/**
 * Cria um elemento de seleção do tipo "range"
 * Abaixo um exemplo de como usar nas views:
 * <code>
 * <?=form_rage('optInputRange', 'optRange','value', '0', '10');?>
 * </code>
 * @param string $id Identificador do input
 * @param string $name Nome do compoente input range
 * @param string $value Valor do componente input range
 * @param integer $min Valor minimo para o range
 * @param integer $max Valor maximo para o range
 * @return string O código HTML para gerar o range
 */ function form_range($id, $name = '', $value = '', $min = '', $max = '') {
    static $form;   

    $form .= '<input name="'. $name .'" class="form-control" type="range" id="'. $id .'" min="'. $min .'" max="'. $max .'" value="'. $value .'" onchange="rangevalue.value=value"/>';
    $form .= '<output id="rangevalue" style="padding-top: -25px; font-size: 15px;">'. $value .'</output>';
 ...
 * @author André Guimarães Peil
 * @param String $nameMap nome do mapa
 ...
 */
function form_MapWithMarker($nameMap = "map_canvas",$nameMarker = 'MarcadorNoMapa', $latitude, $longitude, $zoom = 10,$width = 250, $height = 250, $draggableMarker = false, $mapOptions) {
    
    if ($latitude == null)
        $latitude = -31.747578731227637;
    if ($longitude == null)
        $longitude = -52.32838670702267;
    if($zoom == null){
        $zoom = 10;
    }
    ...
    $script .= "    <script type=\"text/javascript\">
                    ...
                    var latlng = new google.maps.LatLng(" . $latitude . "," . $longitude . ");
                    var zoom = ".$zoom.";
                    var myOptions =". setMapOptions($mapOptions); // setMapOptions ==> seta as opções do mapa;

    $script .="$nameMap = new google.maps.Map(document.getElementById(\"$nameMap\"), myOptions);
    ...
    $script .= "google.maps.event.addListener($nameMarker, 'click', toggleBounce);
                    google.maps.event.addListener($nameMarker, 'dragend', function() {
                    latitude = $nameMarker.getPosition().lat();
                    longitude = $nameMarker.getPosition().lng();
                  ...
                  function toggleBounce() {
                    if ($nameMarker.getAnimation() != null) {
                        $nameMarker.setAnimation(null);
                    } else {
                        $nameMarker.setAnimation(google.maps.Animation.BOUNCE);
                        }
                    }
                    </script>";
    $script .= "<div id=\"$nameMap\" style=\"height: " . $height . "px; width: " . $width . "px;\" load=init() ></div>";
    return $script;
}

/**
 * Seta as opções do mapa
 * cada mapa instanciado possui uma diferença entre opções alguns necessitam de controle de zoom outros não
 * Nesta função permite ao desenvolvedor setar as informações necessárias para a sua aplicação.
 * @author André Guimarães Peil
 * 
 * @param type $opcao ==> opção de mapa
 * @return string :: Retorna uma string contendo as infomações necessárias para o mapa;
 */
function setMapOptions($opcao){
 ...
 * @return Retorna o style, script e a div com id "map_canvas"
 */
function form_MapForPeopleGrid($nameMarker = 'MarcadorNoMapa', $latitude = -15.876809064146757, $longitude = -47.900390625, $positions = '', $mapTipe = 'map', $draggableMap = true, $draggableMarker = false, $zoom = 15) {
    $mapTipe = strtoupper($mapTipe);
    switch ($mapTipe) {
        case 'HYBRID':
        case 'ROADMAP':
    ...
    $script = '<script src="http://maps.google.com/maps/api/js?sensor=false" 
               ...
               <div id="map" style="min-width: 600; min-height: 400;"></div>
